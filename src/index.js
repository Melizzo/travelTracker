  ...
  if (e.target.id === "delete-trip-button") {
    ...
    ApiFetch.deleteSingleTrip({
      ...
      .catch((err) => {
        console.log(err);
        document
          .getElementById("travel-agency-delete-trip")
          .requestFullscreen();
        alert("Error - trip id not found!");
      });
  }
  if (e.target.id === "search-for-traveler") {
    let name = document.getElementById("search-traveler-name-travel-agent")
      .value;
    const searchedtraveler = travelAgent.findSingleTraveler(name);
    traveler = new Traveler(
      {
        id: searchedtraveler.travelerFound.id,
    ...
    domUpdates.displaySearchedTraveler(traveler, travelAgent);
  }
});
